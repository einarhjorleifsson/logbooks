...
# seeding ----------------------------------------------------------------------
SCHEMA <- "adb"
dictionary <- duckdbfs::open_dataset("data/dictionary.parquet") |> collect() |>
  filter(schema == SCHEMA)

# trip -------------------------------------------------------------------------
trip <- duckdbfs::open_dataset("data-dump/logbooks/adb/trip_v.parquet") |> collect() |>
...
# base -------------------------------------------------------------------------
base <-
  duckdbfs::open_dataset("data-dump/logbooks/adb/station_v.parquet") |> collect() |>
  wk_translate(dictionary) |>
...
mobile_aux <-
  duckdbfs::open_dataset("data-dump/logbooks/adb/trawl_and_seine_net_v.parquet") |> collect() |>
...
dredge_aux <-
  duckdbfs::open_dataset("data-dump/logbooks/adb/dredge_v.parquet") |> collect() |>
  wk_translate(dictionary) |>
...
static_aux <-
  duckdbfs::open_dataset("data-dump/logbooks/adb/line_and_net_v.parquet") |> collect() |>
  wk_translate(dictionary) |>
  inner_join(base |> select(.sid, gid, t3, t4), by = ".sid") |>
  mutate(
...
## traps -----------------------------------------------------------------------
trap_aux <-
  duckdbfs::open_dataset("data-dump/logbooks/adb/trap_v.parquet") |> collect() |>
  wk_translate(dictionary) |>
  inner_join(base |> select(.sid, t3, t4), by = ".sid") |>
  mutate(
    dt          = as.numeric(difftime(t4, t3, units = "hours")),
...
seine_aux <-
  duckdbfs::open_dataset("data-dump/logbooks/adb/surrounding_net_v.parquet") |> collect() |>
  wk_translate(dictionary) |>
...
# catch ------------------------------------------------------------------------
  ...
  duckdbfs::open_dataset("data-dump/logbooks/adb/catch.parquet") |> collect() |>
  wk_translate(dictionary) |>
...
# save -------------------------------------------------------------------------
trip    |> duckdbfs::write_dataset("data-raw/logbooks/adb/trip.parquet")
station |> rename(gid_old = gid) |> duckdbfs::write_dataset("data-raw/logbooks/adb/station.parquet")
catch   |> duckdbfs::write_dataset("data-raw/logbooks/adb/catch.parquet")
